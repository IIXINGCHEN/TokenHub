...
| 模型目录 | 创建对外 API 模型、选择初始 Provider 线路并设置统一对客价格 |
...
| **模型目录** | 只管理提供给业务应用的对外模型，即 API 契约。新建时可选择零个或多个可用的已引入 Provider 模型，同步生成初始路由。这里的价格是统一对客价格，不随实际命中的 Provider 路由变化。 |
| **Routing Policies** | 管理对外模型的 Provider 映射，并细调优先级、权重、项目作用域、流量分配和故障转移策略。 |

各区域的职责仍然分开：先添加 Provider 并引入库存；然后在模型目录中一次创建对外模型、选择初始 Provider 线路并设置统一对外价格。之后再使用 Routing Policies 配置高级流量策略。也可以暂不选初始线路，将对外模型保存为待映射草稿。例如，可以向客户端开放 `DeepSeek`，但将它路由到 `OpenAI Production / gpt-4.5`。同一个 Provider 模型可以支持多个对外别名，一个对外模型也可以路由到多个 Provider。
