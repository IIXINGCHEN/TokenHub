...
| 模型目录 | 发布对外 API 模型名、能力和统一对客价格 |
| Routing Policies | 用优先级、权重和故障转移策略把对外模型映射到已引入的 Provider 模型 |
...
| **模型目录** | 只管理提供给业务应用的对外模型，即 API 契约。这里的价格是统一对客价格，不随实际命中的 Provider 路由变化。 |
| **Routing Policies** | 将对外模型显式映射到一个或多个 Provider 已引入的模型，并配置流量分配与故障转移策略。 |

三个步骤有意分开：添加 Provider 并引入库存、创建对外模型、再配置模型路由。例如，可以向客户端开放 `DeepSeek`，但将它路由到 `OpenAI Production / gpt-4.5`。同一个 Provider 模型可以支持多个对外别名，一个对外模型也可以路由到多个 Provider。
