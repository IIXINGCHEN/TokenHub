...
响应中的 `watermark` 标识已完成的数据库快照。在 PostgreSQL 上，新请求日志保存其事务 ID；升级时则会为冻结的历史记录分配按事件时间排列、互不相同且位于新事务 ID 之前的检查点值。检查点停在快照中最早的活跃事务之前，因此后续事务提交不需要等待共享的分析行锁。SQLite 则在数据库原有的单写者模型内使用事务序号。检查点还会停在第一条 `occurred_at` 大于等于 `to` 的已提交匹配请求之前，因此后续推进 `to` 时不会跳过已经提交的未来事件。即使快照没有匹配行也会返回 watermark。Agent 必须先处理所有页面，成功后再把 watermark 持久化。下一轮使用 `after=<已提交 watermark>`：
