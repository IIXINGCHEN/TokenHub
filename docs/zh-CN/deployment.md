...
统一代理支持可选的 Basic 认证，密码会加密保存，API 与控制台只显示掩码。保存时只校验配置语法；「测试代理连接」使用当前未保存表单和已有 Provider，仅验证代理 TCP/TLS、认证、CONNECT 与目标 TLS（使用系统 CA），不会发送 Provider 凭据或模型请求。走代理的目标属于运维控制的代理信任边界；直连及 `NO_PROXY` 命中的请求仍执行 TokenHub DNS/IP 出站校验。代理失败按平台出口故障处理，不会惩罚 Provider 资源，也不会触发路由故障转移。多实例会在五秒内加载共享配置，数据库临时读取失败时继续使用上一份有效配置。
