...
- A managed upgrade runs the target release's own binary against the database first — a read-only preflight (`db verify`) and then the pending expand migrations (`db migrate`) — and only activates the target once both pass. If the activated release then fails its first boot, the previous release is re-activated automatically once (only when the upgrade ran no contract and the previous release's compatibility record covers the database state); a second failure stops version switching for operator recovery.
...
- CI runs the PostgreSQL integration suite and an N-1 two-way contract: the previous release binary creates a database, the current release adopts it and reports ready, the previous release boots again on the adopted database and completes the CRUD contract through its own API (auth, project & team, API key, provider, model & route, one gateway request, audit writes), and the current release returns to serving it and reads every write the old release made. The N-1 contract runs for both SQLite and PostgreSQL, and legacy adoption is additionally validated with the real v0.5.0 fixture. The legacy shape is pinned by the committed immutable N-1 schema fixtures (`backend/internal/dbschema/fixtures/`); CI checks the previous release's database against them with `go run ./cmd/n1check` before adoption (ADR 0005).
