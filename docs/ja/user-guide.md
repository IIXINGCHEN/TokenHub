...
TokenHub は Provider 編集画面の明示的な実テスト結果から画像生成機能を記録します。未テストのアカウントとテストに失敗したアカウントは、どちらもユーザートラフィックの対象外です。実画像の応答に成功した場合だけ対応済みとなり、利用不可のアカウントは同じテストを再実行して復旧できます。TokenHub は回復確認のための画像をバックグラウンドで自動生成しません。

まず、`codex-gpt-image-2` を OpenAI Codex Provider にマッピングし、上流モデルを `gpt-image-2` に設定した有効なルートを構成します。このルートでは、他のモデルと同じ優先度、重み、プロジェクト範囲、指定リソース、リソースグループを利用できます。ルートが存在し、一致する正常な Codex アカウントが明示的な画像生成テストに合格した場合にのみ、`codex-gpt-image-2` が `GET /v1/models` に表示されます。上記の Codex クライアント互換マッピングを除き、別の `gpt-image-2` モデルは OpenAI API Provider を使用し、Codex サブスクリプション枠を消費しません。

Codex サブスクリプションアカウントが既に存在する環境をアップグレードすると、TokenHub は関連する Provider ごとに互換性のある Provider レベルの画像ルートを一度だけ作成します。管理者はその後、通常どおりルートを編集または削除でき、意図的に削除したルートが起動時の移行で再作成されることはありません。
