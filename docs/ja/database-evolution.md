...
- 管理アップグレードは、まず対象リリース自身のバイナリをデータベースに対して実行します（読み取り専用の事前確認 `db verify`、続いて保留中の expand マイグレーション `db migrate`）。両方の成功後にのみ対象リリースをアクティブ化します。アクティブ化したリリースの最初の起動が失敗した場合は、前リリースを自動的に一度だけ再アクティブ化します（アップグレード中に contract を実行しておらず、前リリースの互換性記録がデータベース状態をカバーしている場合のみ）。2 回目の失敗ではバージョン切り替えを停止し、運用者の復旧に委ねます。
...
- CI は PostgreSQL 統合スイートと N-1 双方向契約を実行します。旧リリースバイナリがデータベースを作成し、現在のリリースが採用して準備完了を報告し、旧リリースが採用済みデータベースで再起動して自らの API で CRUD 契約（認証、プロジェクトとチーム、API key、Provider、Model と Route、1 回のゲートウェイリクエスト、監査書き込み）を完了し、現在のリリースが戻ってサービスを再開し、旧リリースが行ったすべての書き込みを読み取れます。N-1 契約は SQLite と PostgreSQL の両方で実行され、レガシー採用は実 v0.5.0 fixture でも追加検証されます。レガシー形状はコミットされた不変 N-1 スキーマ fixture（`backend/internal/dbschema/fixtures/`）で固定され、CI は採用前に `go run ./cmd/n1check` で旧リリースのデータベースを照合します（ADR 0005）。
