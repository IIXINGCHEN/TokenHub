...
データベース起動時の移行は、既存の Codex サブスクリプション Provider に不足している Provider レベルの `codex-gpt-image-2` ルートを一度だけ作成します。永続化された移行リビジョンにより、管理者が後から削除したルートがその後の起動で再作成されることはありません。
