...
TokenHub は `GET /metrics` で Prometheus メトリクスを公開します。このエンドポイントは常に認証を要求します。メトリクスにはモデル名、Provider とリソースの識別子、コストが含まれるため、匿名アクセスは許可されません。`Authorization: Bearer <token>` を送信してください。トークンは `TOKENHUB_METRICS_TOKEN` を使用し、未設定の場合は管理者トークンにフォールバックします。Prometheus のスクレイプ設定に管理者資格情報を置かずに済むよう、専用トークンの設定を推奨します。クエリ文字列でのトークン指定はアクセスログに残るため拒否されます。
