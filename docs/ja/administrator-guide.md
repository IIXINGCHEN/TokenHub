...
| Model Directory | 外部 API モデルを作成し、初期 Provider ルートと統一された顧客向け価格を設定します |
...
| **Model Directory** | アプリケーション向け API 契約となる外部モデルだけを管理します。新規作成時に利用可能な取り込み済み Provider モデルを 0 件以上選択し、初期ルートを同時に作成できます。ここで設定する価格は統一された顧客向け価格で、実際に選択された Provider ルートには左右されません。 |
| **Routing Policies** | 外部モデルの Provider マッピングを管理し、優先度、重み、プロジェクトスコープ、トラフィック配分、フェイルオーバー戦略を調整します。 |

各領域の責務は引き続き分離されています。まず Provider を追加してインベントリを取り込み、次に Model Directory で外部モデルの作成、初期 Provider ルートの選択、統一外部価格の設定を一度に行います。高度なトラフィック設定は、その後 Routing Policies で行います。初期ルートを選択せず、マッピング待ちの下書きとして外部モデルを保存することもできます。たとえば、外部モデル `DeepSeek` を公開しながら `OpenAI Production / gpt-4.5` へルーティングできます。同じ Provider モデルを複数の外部エイリアスに使用でき、1 つの外部モデルを複数 Provider にルーティングすることもできます。
