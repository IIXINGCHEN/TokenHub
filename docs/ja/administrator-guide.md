...
| Model Directory | 外部 API のモデル名、機能、統一された顧客向け価格を公開します |
| Routing Policies | 外部モデルを取り込み済みの Provider モデルへ、優先度、重み、フェイルオーバー戦略で割り当てます |
...
| **Model Directory** | アプリケーション向け API 契約となる外部モデルだけを管理します。ここで設定する価格は統一された顧客向け価格で、実際に選択された Provider ルートには左右されません。 |
| **Routing Policies** | 外部モデルから、1 つ以上の Provider に取り込み済みモデルへの明示的なマッピングと、トラフィック配分・フェイルオーバー戦略を管理します。 |

手順は意図的に分離されています。Provider を追加してインベントリを取り込み、外部モデルを作成してから、そのルートを設定します。たとえば、外部モデル `DeepSeek` を公開しながら `OpenAI Production / gpt-4.5` へルーティングできます。同じ Provider モデルを複数の外部エイリアスに使用でき、1 つの外部モデルを複数 Provider にルーティングすることもできます。
