...
<p align="center">
  TokenHub は、AI 時代のエンタープライズ Token Governance 基盤です。モデルルーティング、権限管理、Token コスト最適化、Provider 照合に集中し、アカウント転送を中核にはしません。
</p>
...
TokenHub は「アカウント転送ツール」を目指していません。アカウント転送にはすでに成熟したプラグインや方式があります。TokenHub はそれらを Provider 抽象とプラグイン型の接続層に収め、製品の中心には置きません。
...
多くのオープンソース AI ゲートウェイは Provider 集約やアカウント転送に重点を置いています。つまり、1 つのエンドポイントから複数の上流を呼び出したり、サブスクリプションアカウントやチャネルアカウントを統一入口として包んだりする仕組みです。これは開発者のモデル接続には役立ちますが、企業運用の課題をそれだけで解決できるわけではありません。TokenHub は、その不足しているガバナンス層を中心に設計されています。
...
Provider は接続境界であり、製品の中心ではありません。アカウント転送、サブスクリプションチャネル、ホスト型 API、ローカルモデル、カスタム上流はすべて Provider 抽象の背後に置き、TokenHub の中心をエンタープライズ Token ガバナンスに保ちます。
