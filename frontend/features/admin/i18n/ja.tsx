export const jaTranslations: Record<string, string> = {
    ...
    "主团队用于默认责任和成本归属。": "プライマリチームは既定の責任とコスト配賦に使用されます。", "用户可通过任一团队关联获得项目权限；多条权限按最高项目角色合并。": "ユーザーは所属するいずれかのチームから Project 権限を取得でき、複数の権限は最上位の Project ロールに統合されます。", "默认团队继续承担成本、审批和责任归属；保存项目后可在详情中关联更多团队。": "プライマリチームはコスト、承認、責任を引き続き担います。保存後に Project 詳細から追加チームを関連付けられます。",
  };
