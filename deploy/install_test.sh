...
printf '%s\n' "$*" >>"$FAKE_CALL_LOG"
up_has_run=false
if grep -q 'up -d --remove-orphans --no-build --pull never' "$FAKE_CALL_LOG"; then
  up_has_run=true
fi
...
assert_not_contains "$(<"$CALL_LOG")" "up -d --remove-orphans --no-build --pull never"
...
assert_contains "$(<"$CALL_LOG")" "up -d --remove-orphans --no-build --pull never"
FAKE_PULL_STATUS=0
